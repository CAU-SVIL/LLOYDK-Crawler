...
def ytb_channel_data_func(driver):
    ...
    for channel_name in channel_name_list:
        ...
        channel_data = {'name': '', 'url': '','date': '', 'subscriber': '', 'total_views':'', 'video_counts': '', 'video_url': []}
...
def ytb_video_data_func(options, service, channel, video_urls):
    
    ytb_video_data={'channel_name': channel, 'video_list': []}
    video_list=[]
    driver=webdriver.Chrome(options=options, service=service)
    
    for video_url in video_urls: 
        video_data = {'video_url':video_url ,'date': '', 'title': '', 'view': '', 'comment': ''}
        video_error={'video_url': video_url,'date': '-1', 'title': '-1', 'view': '-1', 'comment': '-1'}
        print(video_url)
        try:
    ...
    driver.quit()
    ytb_video_data['video_list']=video_list
#    ytb_video_data=json.dumps(ytb_video_data)
...
#     data=ytb_channel_data_func(driver)
    channel_data=ytb_channel_data_func(driver)
    video_data=ytb_video_data_func(options, service ,channel_data[0]['name'], channel_data[0]['video_url'])
